<!--//
            $.getJSON("https://www2c.cdc.gov/podcasts/feed.asp?feedid=66&format=json&callback=?",
             function (data) {
                                        var count_items = 0
var monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
                                                                      var list = $('<ul class="list-bullet"/>');
                                                $('#rss-outbreaktravelers').append(list);
...
newItem.append($('<span class="item-pubdate" />').html("Announced " + monthNames[d.getMonth()] + " " + d.getFullYear()));
                                                            //newItem.append($('<span class="item-pubdate" />').html(d.toLocaleDateString ()));
                                                            /*newItem.append($('<span class="item-description" />').html(entry.description));*/
                                                            list.append(newItem);
                                                                                                      count_items = count_items + 1 
                                                                                             }//end if greater than 6
                });
            });
//--> 