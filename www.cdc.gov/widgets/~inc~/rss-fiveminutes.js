...
$.getJSON("https://www2c.cdc.gov/podcasts/feed.asp?feedid=282&maxnumber=0&pagesize=9999&format=json&callback=?",
                function (data) {
                                var count_items = 0;
                                var list = $('<ul />');
                                 $('#rss-fiveminutes').append(list);
                                $.each(data.entries, function (i, entry) {
                                    var category = (entry.categories && entry.categories.length > 0) ? entry.categories[0] : '';
		var dtx=entry.pubdate.substr(0, 4);
                                               if (category =="") {
		                                     
                                                                var newItem = $('<li class="span8 pull-left"/>');
