...
var s_linkDownloadFileTypes="exe,zip,jpg,wav,mp3,mp4,mov,mpg,avi,doc,docx,xlsx,pptx,pdf,xls,rm,wmv,rtf,ppt,pps,swf,prc,xml,sas,epub,ics,vcs,ris,msi,csv";
var s_linkInternalFilters="javascript:,cdc,file:///,outbind,//www/,mid://,cdc";
...
function s_doPlugins() {
	var ds=["bam.gov","cdc.gov","5aday.gov","mid://","http://198.246.96.2","outbind:"];
	var dlh=s_wd.location.href.toLowerCase();
	var cond=false;
	for(var i=0;i<ds.length;i++) {
		if(dlh.indexOf(ds[i])!=-1) cond=true;
	}
	if(cond==false) s_vpr("s_prop10",dlh);

	/* check for trade mark symbol */
	spn=escape(s_pageName);
...
s_vp_getCustomPagePath("s_prop15",s_vp_getValue("s_campaign"),"s_p8","");
}
