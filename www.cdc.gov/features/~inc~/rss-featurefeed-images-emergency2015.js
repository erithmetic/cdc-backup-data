...
$.getJSON("https://www2c.cdc.gov/podcasts/feed.asp?feedid=198&maxnumber=0&pagesize=9999&daterange=2&startdate=1/1/15&enddate=12/31/15&format=json&callback=?",
                function (data) {
                                var count_items = 0;
                                var list = $('<ul />');
                                 $('#rss-featurefeed-images-emergency2015').append(list);
                                $.each(data.entries, function (i, entry) {
                                    var category = (entry.categories && entry.categories.length > 0) ? entry.categories[0] : '';
		var dtx=entry.pubdate.substr(0, 4);
                                               if (category =="Emergency Preparedness & Response") {
